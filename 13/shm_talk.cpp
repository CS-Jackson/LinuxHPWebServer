    ...
    while(!stop_child)
    {
        ...
        for(int i = 0; i < number; i++)
        {
            ...
            if((sockfd == connfd) && (events[i].events & EPOLLIN))
            {
                ...
                //因此，各个客户连接的读缓存是共享的。
                ret = recv(connfd, share_mem + idx * BUFFER_SIZE, '\0', BUFFER_SIZE);
                if(ret < 0)
                {
                    if(errno != EAGAIN)
                    {
                ...
                {
                    //成功读取客户数据后通知主进程（管道）来处理
                    send(pipefd, (char *)&idx, sizeof(idx), 0);
                }
            }
            //主进程通知本进程（管道）将第client个客户的数据发送到本进程负责的客户端
    ...
    while(!stop_server)
    {
        ...
        for(int i = 0; i < number; i++)
        {
            ...
            else if(events[i].events & EPOLLIN)
            {
                ...
                {
                    //向除负责第child个客户连接的子进程之外的其他子进程发送消息，通知它们有客户数据要写
