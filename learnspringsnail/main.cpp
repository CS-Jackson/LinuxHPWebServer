...
int main( int argc, char* argv[] )
{
    ...
    while( tmp2 = strpbrk( tmp, "\n" ) )
    {
        *tmp2++ = '\0';
        ...
        else if( tmp3 = strstr( tmp, "<name>" ) )
        {
            ...
            std::cout << tmp_hostname << std::endl;
            *tmp4 = '\0';   //原来tmp4指向"<"，然后这里截断了后面的</name>。
            std::cout << tmp_hostname << std::endl;
    ...
    const char* ip = balance_srv[0].m_hostname; //Listen的IP地址
    int port = balance_srv[0].m_port;           //Listen的端口
